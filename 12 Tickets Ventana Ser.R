...
# ------------------------------------------------------------------------------
# Paso 1: Obtener registros nuevos y actualizados
# ------------------------------------------------------------------------------
query = """
    SELECT 
        h.id_key, 
    ...
    WHERE 
        s.id_key IS NULL  -- Registros nuevos
        OR (
...
if not nuevos_registros.empty:
    ...
    nuevos_registros["time_end_status"] = nuevos_registros.apply(
        ...
        if (row["code_estado_ticket"] != "6" and pd.isna(pd.to_datetime(row["original_time_end_status"], errors="coerce"))) 
    ...
    # Paso 3: UPSERT
    # --------------------------------------------------------------------------
    ids_para_actualizar = nuevos_registros["id_key"].tolist()
    if ids_para_actualizar:
        ids_str = ",".join(map(str, ids_para_actualizar))
        conn.execute(f"DELETE FROM hist_status_tickets_sw WHERE id_key IN ({ids_str})")
