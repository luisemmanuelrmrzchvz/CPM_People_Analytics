...
# Ejecutar código Python para crear el dashboard (versión corregida de formato numérico)
py_run_string("
import pandas as pd
...
# Intento de configurar locale (no es estrictamente necesario para el formateo que usamos)
try:
...
# Función para formatear números con comas para miles (formato México: 1,800)
def format_number(value, is_currency=False, is_percentage=False, is_integer=False):
    try:
        if pd.isna(value):
            return '0'
        
        # Para porcentajes, mostrar un decimal
        if is_percentage:
            return f'{value:,.1f}%'
...
def create_indicator_section(fig, grid_spec, row, col, data, metric_name, subtitle):
    ...
    # CUADROS 3-4: Gráfico de tendencia
...
# Gráfico de tendencia con doble eje vertical
def create_trend_chart(ax, metric_data, metric_name):
    ...
    # Formateador personalizado para usar comas para miles y punto para decimales
    def volume_formatter(x, p):
        if x == 0:
            return '0'
        # Si x es entero, no mostrar decimales; si tiene fracción, mostrar 1 decimal
...
# Crear imágenes para cada grupo
for group_name, metrics in indicator_groups.items():
    ...
    output_dir = 'C:/Users/racl26345/Documents/Reportes Automatizados/Observatorio Digital'
