...
> # 3d) Brechas entre pares por departamento (versión optimizada)
  > cat('Calculando brechas promedio por departamento y puesto...\n')
Calculando brechas promedio por departamento y puesto...
> 
  > # Calcular vector promedio por grupo
  > vectores_promedio <- vectores %>%
...
---
  Backtrace:
  ▆
1. ├─vectores %>% group_by(Nivel_3, Puesto_Generico) %>% ...
2. ├─dplyr::summarise(...)
3. ├─dplyr::group_by(., Nivel_3, Puesto_Generico)
4. └─dplyr:::group_by.data.frame(., Nivel_3, Puesto_Generico)
