...
# ---------------------------
# 8) Resumen general y recomendación automática
# ---------------------------
cat("\n=========================================\n")
cat("RESUMEN GLOBAL DE ESTIMACIONES\n")
cat("Resultados guardados en:", output_dir, "\n")

# Mostrar recomendaciones por grupo analizando estabilidad y baseline
for (k in names(summary_results)) {
  res <- summary_results[[k]]
  if (is.null(res)) next
  cat("\n----", k, "----\n")
  cat("Baseline mediana:", paste(res$baseline, collapse = " / "), "\n")
  stab <- res$stability
  if (!is.na(stab)) {
    cat("Estabilidad bootstrap (mayor es mejor):", round(stab, 3), "\n")
    if (stab >= 0.6) cat(" Recomendación: Usar modelo (predicción basada en ML).\n")
    else cat(" Recomendación: Modelo débil; usar percentiles/rangos.\n")
  } else {
    cat("Estabilidad no calculada: usar percentiles/rangos.\n")
  }
}

cat("\n=========================================\n")
cat("FIN DEL PROCESO. Revisa archivos en carpeta de salida.\n")
