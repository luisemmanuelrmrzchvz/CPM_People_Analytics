...
if (file.exists(ruta_archivo)) {
  ...
  # Convertir columna 17 (timestamp) a fecha eliminando la hora
  if (inherits(datos[[17]], "POSIXt")) {
    datos[[17]] <- as.Date(datos[[17]])
  } else {
    datos[[17]] <- as.Date(as.POSIXct(datos[[17]], format = "%d/%m/%Y %I:%M:%S %p", tz = "UTC"))
  }

  # Convertir la columna 6 a fecha en formato correcto
  ...
  print(paste("Archivo no encontrado:", nombre_archivo))
}
